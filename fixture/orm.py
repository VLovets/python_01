...
class ORMFixture:
    ...
    def convert_to_groups_model(self, groups):
        def convert(group):
            return GroupNew(id=str(group.id), name=group.name, header=group.header, footer=group.footer)
    ...
    @db_session
    def check_group_in_list(self, group):
        #with db_session:
        return self.convert_to_groups_model(list(select(g for g in ORMFixture.ORMGroup if g.id == group.id or g.name == group.name)))
    ...
    def get_contacts_not_in_group(self, group):
        orm_group = list(select(g for g in ORMFixture.ORMGroup if g.id == group.id or g.name == group.name))[0]
        return self.convert_to_contacts_model(
